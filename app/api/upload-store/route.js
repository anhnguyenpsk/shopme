...
export async function POST(request) {
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'store', 'avatar')
    await mkdir(uploadDir, { recursive: true })
    const fullPath = path.join(uploadDir, unique)

    await writeFile(fullPath, buffer)

    const publicPath = `/uploads/store/avatar/${unique}`
    return NextResponse.json({ path: publicPath }, { status: 200 })
  } catch (err) {
    console.error('Upload store avatar error', err)
