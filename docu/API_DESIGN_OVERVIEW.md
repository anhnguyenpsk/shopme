...
## 10. Module AI Chatbot (New)

### 10.1. API Chat với AI (API-072)

|              | Nội dung                                                                                                        |
| :----------- | :-------------------------------------------------------------------------------------------------------------- |
| **Endpoint** | `/api/chat`                                                                                                     |
| **Method**   | `POST`                                                                                                          |
| **Request**  | `{ "messages": [{ "role": "user", "content": "..." }, ...], "sessionId": "..." }`                               |
| **Response** | `{ "role": "assistant", "content": "...", "id": "..." }`                                                        |
| **Xử lý**    | Gửi tin nhắn đến mô hình AI (Gemini), xử lý intent (tìm kiếm, hỏi đáp), gọi tool nếu cần và trả về câu trả lời. |

### 10.2. API Lịch sử Chat (API-073)
...
| **Response** | `{ "data": [...], "status": 200 }`        |
| **Xử lý**    | Lấy lại lịch sử hội thoại của phiên chat. |
