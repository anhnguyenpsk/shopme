...
export async function buildCartSnapshot(cartItems, { prismaClient = prisma } = {}) {
  ...
  const variants = variantIds.length > 0
    ? await prismaClient.productVariant.findMany({
  ...
  for (const rawItem of cartItems) {
    ...
    // Resolve Price and Quantity based on Variant
    ...
    const quantity = Math.min(requestedQty, quantityAvailable ?? requestedQty);
    ...
    lines.push({
      ...
      product,
      variant // Include variant in line info if needed
    });
  }
